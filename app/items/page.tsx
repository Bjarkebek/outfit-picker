...
// Used to define data from the database
type Item = {
  id: string;
...
/* --- Create form state --- */
// Hook to handle state for creating a new item
function useCreateState() {
  ...
  return {
    ...
    reset: () => {
      ...
      setTypeValue(''); setSeason(''); setFile(null);
    }
  };
}

// Main component for the items page
export default function ItemsPage() {
  // State for all items and loading indicator
    ...
    try {
      ...
      if (c.file) {
        ...
        const { data: upData, error: upErr } = await supabase.storage
          .from('outfit-images').upload(path, c.file);
      ...
      // Error on creation
      alert('Error on creation: ' + e.message);
  ...
  async function saveEdit() {
    ...
    try {
      ...
      // Should we upload a new file?
      if (eFile) {
        // Get existing image_path to delete old file
        const { data: one } = await supabase.from('item')
          .select('image_path').eq('id', editId).single();
        if (one?.image_path) {
        ...
        const path = `items/${crypto.randomUUID()}_${safeName}`;
        const { data: upData, error: upErr } = await supabase.storage
          .from('outfit-images').upload(path, eFile);
        if (upErr) throw upErr;
      ...
      const { error: upErr } = await supabase.from('item')
        .update(updatePayload).eq('id', editId);
      if (upErr) throw upErr;

      // Fetch updated list and finish editing
      await loadItems();
      cancelEdit();
    } catch (e: any) {
      // Error on update
  ...
  async function onDelete(id: string) {
    ...
    setLoading(true);
    try {
      // Get image_path to delete image from storage
      const { data: one } = await supabase
        .from('item').select('image_path').eq('id', id).single();
      ...
      // Error at deletion
      alert('Error at deletion: ' + e.message);
    } finally { setLoading(false); }
  }
      ...
      <section className={`${cardClass} mt-6`}>
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input className={inputClass} placeholder="e.g. jeans, skirt, sneakers…"
              value={c.typeValue} onChange={(e) => c.setTypeValue(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label className="text-sm">Season</Label>
            <Select value={c.season || 'none'} onValueChange={(v) => c.setSeason(v === 'none' ? '' : v)}>
          ...
          <div key={i.id} className={`${cardClass}`}>
            ...
            {editId === i.id && (
              <div className="mt-5 grid gap-4 md:grid-cols-3 border-t pt-5 border-gray-200 dark:border-white/10">
                ...
                <div className="flex flex-col gap-1.5">
                  <Label className="text-sm">Season</Label>
                  <Select value={eSeason || 'none'} onValueChange={(v) => setESeason(v === 'none' ? '' : v)}>
                    <SelectTrigger className="w-full"><SelectValue placeholder="— select —" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">— select —</SelectItem>
                      {SEASONS.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
